...
const Dupoin = ({ onAction }) => {
  ...
  return (
    <section id="dupoin" className="py-24 bg-[#060b13] overflow-hidden">
      <div className="max-w-[1440px] mx-auto px-6 md:px-12">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-16 mb-32">
          <motion.div 
            ...
            className="lg:w-1/3"
          >
            <img src={dupLogo} alt="Duopin Logo" className="w-48 mx-auto lg:mx-0 opacity-80" />
          </motion.div>
        ...
        <div className="mb-32">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            >
              <div className="absolute -inset-1 bg-[#00d1ff]/20 rounded-[2.5rem] blur-2xl group-hover:bg-[#00d1ff]/30 transition-all duration-1000"></div>
            ...
            <div className="lg:col-span-8 grid grid-cols-2 md:grid-cols-4 gap-8">
              {standardAwards.map((award, index) => (
                <motion.div 
                  ...
                  viewport={{ once: true }}
                  transition={{ ...slowTransition, delay: 0.4 + (index * 0.15) }}
                  className="flex flex-col items-center text-center group"
                >
                  <div className="relative mb-4">
                    <img src={award.src} alt={award.label} className="w-24 md:w-32 opacity-40 group-hover:opacity-100 transition-all duration-700 group-hover:scale-110" />
                  </div>
                  <h4 className="text-white text-[10px] md:text-xs font-bold leading-tight uppercase tracking-widest max-w-[110px]">
                    {award.label} <br/> <span className="text-[#00d1ff]">{award.year}</span>
                  </h4>
                </motion.div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-24">
          {advantages.map((item, index) => (
            <motion.div
              ...
              className="p-8 rounded-[2.5rem] bg-white/[0.02] border border-white/5 hover:border-[#00d1ff]/30 transition-all duration-500 cursor-default"
            >
              <div className="mb-6 bg-black/40 w-14 h-14 rounded-2xl flex items-center justify-center border border-white/5">{item.icon}</div>
        ...
        {/* Updated CTA Button: Blue to Deep Blue Transition */}
        <motion.div 
        ...
        >
          <button 
            onClick={onAction}
            className="group relative flex items-center gap-4 px-12 py-5 rounded-full bg-[#00d1ff] text-black font-black text-xl uppercase tracking-widest overflow-hidden transition-all hover:shadow-[0_0_50px_rgba(0,209,255,0.3)]"
          >
            ...
            {/* Hover overlay: Transitions from current blue to a darker shade */}
            <div className="absolute inset-0 bg-[#00a3cc] translate-x-[-100%] group-hover:translate-x-0 transition-transform duration-500 ease-[0.22, 1, 0.36, 1]" />
          </button>
          <p className="text-gray-500 text-xs uppercase tracking-[0.4em] font-bold">Official Partnership execution</p>
        </motion.div>
